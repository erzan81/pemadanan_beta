...
class MMatching extends CI_Model {
   ...
   function get_kolom_pemadanan($id_upload){
    ...
    public function metode_pemadanan_upd($save){
        $results = '';
        $this->pblmig_db = $this->load->database('pblmig', true);
        if (!$this->pblmig_db) {
            $m = oci_error();
        ...
        $stid = oci_parse($this->pblmig_db->conn_id, 'BEGIN PEMADANAN_APP.PKG_PEMADANAN.UPD_METODE(:p_instansi_id, :p_id_upload, :p_step_ke, :p_id_kolom, :p_is_proses, :p_is_digit, :p_metode, :p_nilai, :p_atribut, :p_create_by, :p_digit, :out_rowcount, :msgerror); END;');
   ...
   public function init_pemadanan($save){
    ...
    function get_pemadanan($p_id_upload){
        $results = '';
    ...
    function get_pemadanan_detil($p_id_upload, $p_step_ke){
        $results = '';
        ...
        oci_bind_by_name($stid, ':p_id_upload', $p_id_upload,100, SQLT_CHR) or die('Error binding string5');
        oci_bind_by_name($stid, ':p_step_ke', $p_step_ke,100, SQLT_CHR) or die('Error binding string5');
        oci_bind_by_name($stid, ':RetVal', $OUT_DATA,-1, OCI_B_CURSOR) or die('Error binding string1');
    ...
    public function main_pemadanan_job($p_id_upload, $p_step_ke){
        $results = '';
        $this->pblmig_db = $this->load->database('pblmig', true);
        if (!$this->pblmig_db) {
            $m = oci_error();
      ...
      function get_metode_pemadanan_upd($p_id_upload, $p_step_ke){
        $results = '';

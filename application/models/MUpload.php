...
class MUpload extends CI_Model {
   ...
   public function ins_file($save){
        ...
        oci_bind_by_name($stid, ':p_isi_data', $p_isi_data, 100) or die('Error binding string2');
        oci_bind_by_name($stid, ':out_rowcount', $OUT_ROWCOUNT,100, SQLT_CHR) or die('Error binding string3');
        oci_bind_by_name($stid, ':msgerror', $OUT_MESSAGE,100, SQLT_CHR) or die('Error binding string4');      
   ...
   public function new_upload($save){
        ...
        $stid = oci_parse($this->pblmig_db->conn_id, 'BEGIN PKG_UPLOAD.NEW_UPLOAD(:p_instansi_id, :p_nama_file, :p_jns_upload, :p_kolom, :p_create_by, :out_rowcount, :msgerror, :out_id_upload); END;');
        ...
        oci_bind_by_name($stid, ':msgerror', $msgerror,100, SQLT_CHR) or die('Error binding string5');
    ...
    public function get_upload_temp($p_create_by){
        $results = '';
        $this->pblmig_db = $this->load->database('pblmig', true);
        if (!$this->pblmig_db) {
            $m = oci_error();
        ...
        oci_bind_by_name($stid, ':RetVal', $OUT_DATA,-1, OCI_B_CURSOR) or die('Error binding string5');        
    ...
    public function rekap_upload($save){
        ...
        oci_bind_by_name($stid, ':msgerror', $msgerror,100, SQLT_CHR) or die('Error binding string5');
        oci_bind_by_name($stid, ':out_rowcount', $out_rowcount,100, SQLT_CHR) or die('Error binding string5');
    ...
    public function next_upload($save){
        ...
        $stid = oci_parse($this->pblmig_db->conn_id, 'BEGIN  :RetVal := PEMADANAN.PKG_UPLOAD.NEXT_UPLOAD(:p_instansi_id, :p_id_upload, :p_nama_file, :p_jns_upload, :p_create_by, :out_id_upload, :out_rowcount, :msgerror); END;');
        ...
        oci_bind_by_name($stid, ':msgerror', $msgerror,100, SQLT_CHR) or die('Error binding string7');
        oci_bind_by_name($stid, ':out_rowcount', $out_rowcount,100, SQLT_CHR) or die('Error binding string8');
        oci_bind_by_name($stid, ':RetVal', $RetVal) or die('Error binding string9');
    ...
    public function hapus_upload($save){
        ...
        $stid = oci_parse($this->pblmig_db->conn_id, 'BEGIN :RetVal := PEMADANAN.PKG_UPLOAD.HAPUS_UPLOAD(:p_instansi_id, :p_id_upload, :p_delete_by, :msgerror, :out_rowcount); END;');
